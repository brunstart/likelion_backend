...
public class GroupingExample {
    public static void main(String[] args) {
        ...
        complexGrouping.forEach((dept, emps) -> {
            ...
            List<Employee> highSalaryEmp = emps.get(true);
            List<String> highSalEmpNames = highSalaryEmp.stream()
                    .map(Employee::getName)
                    .collect(Collectors.toList());
            System.out.println(" - sal > 4500 (true) : " + highSalEmpNames);

            List<Employee> normalSalaryEmp = emps.get(false);
            List<String> normalSalEmpNames = normalSalaryEmp.stream()
                    .map(Employee::getName)
                    .collect(Collectors.toList());
            System.out.println(" - sal <= 4500 (false) : " + normalSalEmpNames);
        });
    }
